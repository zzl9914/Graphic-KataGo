...
蒸馏与分阶段自对弈写在 [`training_method.md`](training_method.md)。  
...
MCTS 用 policy + value(lead) + stdev（加权备份、方差缩放 cPUCT）。其余辅助头不进搜索。随机图上的讲解 / drone 规划见 §10.1。
...
训练与 UI **必须**加载 `gkt_native`（失败即报错，不回退慢路径）。Windows 入口 `start_train.bat` / `start_ui.bat` 钉死 **Python 3.14**。不要用 `py -3`（可能开到 free-threading 的 `python3.14t.exe`），也不要用 3.13：扩展是 `gkt_native.cp314-win_amd64.pyd`。编译见 [`training_method.md`](training_method.md) §1。
...
- 根 Dirichlet（\(\alpha=0.3\)，\(f=0.25\)）：训练自对弈、Arena、UI。自对弈走子按温度采样（围棋主线 \(\tau=0.1\)）；Arena / UI 仍 `argmax` 访问，sim 固定。Elo：`argmax` 且 `dirichlet_frac=0`。
...
- 叶子对每个座位各做一次视角特征，得到 \(k\) 维 lead；备份时路径上每节点取**该节点执子方**的分量，**不是**二人翻号。价值公式见 [`training_method.md`](training_method.md) 的 `score_lead`（二人即 \((\text{我}-\text{对方})/n\)）。
...
主干宽度与搜索算法固定。训练同时拟合这些头；**进 MCTS 的只有 policy / value / stdev**。
...
单卡（典型 RTX 4050 Laptop 6GB）上，self-play 为多进程：规则/MCTS 在 C++，每进程自己 `predict_batch`。围棋主线为 `1 worker × gpw 32`（每 cycle 32 局），名义 `--sim 256`。CPU 与 GPU 一样：**每个 worker 提交一次**、`n_games=gpw`（1DCNN 约 60MB 权只 pickle 一次/worker，而不是每局一次）。叶子评估 `--selfplay-batch`：`gkt_train_gpu.py` 与 `gkt_dist.py` 默认 64；`gkt_train_cpu.py` 默认 32。`start_train.bat` 不改写该旗标。
