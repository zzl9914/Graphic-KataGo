@echo off
REM =====================================================================
REM  base/cultivate2_mlp - 基础培养2 (MLP)
REM  *** NOT official cross-graph training (that is starter/train_*.bat) ***
REM
REM  After distillation (基础培养 / base/mlp/new.npz), before
...
REM
REM  Output: base/cultivate2/mlp/new.npz
...
echo Interpreter: %PY%
echo Stage: cultivate2 / MLP - graph 0, 20 rounds, 30/5, freeze policy 1-10

if exist "%~dp0cultivate2\mlp\new.npz" (
  echo Resume: base\cultivate2\mlp\new.npz
  set "RESUME=../base/cultivate2/mlp/new.npz"
) else if exist "%~dp0mlp\new.npz" (
  echo Resume: base\mlp\new.npz - distill / 基础培养
  set "RESUME=../base/mlp/new.npz"
) else (
  echo [ERROR] missing distill checkpoint: %~dp0mlp\new.npz
  echo.
  echo  Run base\distill_mlp.bat first, then re-run this bat.
  echo.
  pause
  exit /b 1
)
...
cd /d "%~dp0..\scr"
"%PY%" gkt_train_cpu.py --net mlp --hidden 512 --graphs 0 --rounds 20 --freeze-policy-until-round 10 --value-weight 30 --own-weight 5 --q-lambda 0.5 --lr 1e-4 --temperature 0.1 --sim 256 --workers 1 --gpw 32 --steps 16 --no-arena --buffer-drop-from-round 21 --model-snapshot-rounds 25 --outdir ../base/cultivate2/mlp --resume !RESUME!

echo.
echo cultivate2 done. Model: base\cultivate2\mlp\new.npz
