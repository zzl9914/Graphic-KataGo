...
REM  in base/mlp/ and continues at epoch N+1. No checkpoint = epoch 1.
...
if exist "%~dp0mlp\round*.npz" (
  echo Resume: latest round*.npz under base\mlp
) else (
  echo No round*.npz under base\mlp - starting a new distillation.
)

cd /d "%~dp0..\scr"
"%PY%" distill.py --net mlp --data "../%DATA%" --graph-key 0 --outdir ../base/mlp --hidden 512 --epochs 10 --batch-size 64 --lr 1e-4 --value-weight 30 --own-weight 5 --aug-from-epoch 4 --resume
